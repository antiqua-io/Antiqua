class Archive
  ...
  state_machine :initial => :initialized do
    ...
    after_transition :on => :clone   , :do => :enqueue_clone_worker      # 2
    after_transition :on => :queue   , :do => :enqueue_deploy_key_worker # 1
  end

  def enqueue_archive_worker
    Resque.enqueue ArchiveWorker , :archive_id => id
  end

  def enqueue_clean_worker
    Resque.enqueue ArchiveCleanWorker , :archive_id => id
  end

  def enqueue_clone_worker
    Resque.enqueue ArchiveCloneWorker , :archive_id => id
  end
